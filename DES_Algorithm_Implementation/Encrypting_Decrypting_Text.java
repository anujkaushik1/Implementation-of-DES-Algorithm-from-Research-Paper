...
public class Encrypting_Decrypting_Text {
	
	public static String round_keys[] = new String[16];
	...
	public static String DES(){ 
		...
		33,1,41,9,49,17,57,25 
	  	...
	  	String perm = ""; 
		for(int i = 0; i < 64; i++){ 
			perm += pt.charAt(initial_permutation[i]-1); 
		}  
		...
		for(int i=0; i<16; i++) { 
	    	...
	    	for(int i1 = 0; i1 < 48; i1++) { 
	      		right_expanded += right.charAt(expansion_table[i1]-1); 
	    }
			...
			for(int i1=0;i1<8; i1++){ 
	      		...
	      		String row1= xored.substring(i1*6,1 + i1*6 ) + xored.substring(i1*6 + 5,1 + i1*6 + 5);
	      		int row = convertBinaryToDecimal(row1);
	      		String col1 = xored.substring(i1*6 + 1,1 + i1*6 + 1) + xored.substring(i1*6 + 2,1 + i1*6 + 2) + xored.substring(i1*6 + 3,1 + i1*6 + 3) + xored.substring(i1*6 + 4,1 + i1*6 + 4);
				int col = convertBinaryToDecimal(col1);
				int val = substition_boxes[i1][row][col];
				res += convertDecimalToBinary(val);  
			} 
			
			String perm2 =""; 
			for(int i1 = 0; i1 < 32; i1++){ 
				perm2 += res.charAt(permutation_tab[i1]-1); 
			}
	...
	public static void main(String args[]) {
		String key= "101010101011101100001001000110000010011100110110110011001101101"; 
		System.out.println(pt.length());
		if(pt.length() != 64) {
			
			if(pt.length() < 64) {
				System.out.println("The length of the key should be greater than "+pt.length()+" and must be equal to 64");
			}
			else if(pt.length() > 64) {
				System.out.println("The length of the key should be less than "+pt.length()+" and must be equal to 64");
			}
			return ;
		}
